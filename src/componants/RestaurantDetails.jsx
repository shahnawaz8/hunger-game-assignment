...
export const RestaurantDetails=()=>{
  ...
  const getdata =()=>{
  axios.get(`http://localhost:8080/get-restaurants?_page=${page}&_limit=5`).then((res)=>{
   setdata(res.data)
  ...
  const handlesortacs=()=>{
    axios.get("http://localhost:8080/get-restaurants").then((res)=>{
   ...
   setdata(res.data.sort((a,b)=>{return b.costForTwo - a.costForTwo}))
    ...
    return(
        <div>
            ...
            <button onClick={()=>{axios.get("http://localhost:8080/get-restaurants?rating_gte=4").then((res)=>{setdata(res.data)})}}>4 and above</button>
            <button onClick={()=>{axios.get("http://localhost:8080/get-restaurants?rating_gte=3").then((res)=>{setdata(res.data)})}}>3 and above</button>
            <button onClick={()=>{axios.get("http://localhost:8080/get-restaurants?rating_gte=2").then((res)=>{setdata(res.data)})}}>2 and above</button>
            <button onClick={()=>{axios.get("http://localhost:8080/get-restaurants?rating_gte=1").then((res)=>{setdata(res.data)})}}>1 and above</button>
            ...
            <button onClick={()=>{axios.get('http://localhost:8080/get-restaurants').then((res)=>{setdata(res.data.filter((e)=>{if (e.payment_methods.upi===true) {return true} else{return false}}))})}}>Upi</button>
            <button onClick={()=>{getdata()}}>all</button>
            </div>
         <div>
             {data.map((el)=>
